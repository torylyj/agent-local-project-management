...
- **自动归集**：扫描对话目录，把散落在不同会话里的工作按主题合并成项目视图
- **进度可视化**：顶部统计、Token 月度消耗、按年/月倒序时间线、完成标准与推进建议
- **创作中心**：每日灵感、选题方案、AI 新闻（收藏 / 已读）
- **严格标准**：完成标准未闭环的项目一律不算「已完成」
- **云端同步（进阶）**：状态 / 进度 / 紧急程度跨设备同步，三层自动更新保障

**使用方式**：让本地 Agent 扫描对话目录，把结果写进 `data.json`，页面自动渲染、刷新即更新——完全不用改页面代码。

> 📌 版本口径：`template.html` 为**基线版 v1.3**（数据驱动，轻量开箱即用）；文档 v1.4 描述的进阶能力（搜索 / 看板 / 云端同步 / Token 绑定 / 周报等）属于完整功能版，需按「九、云端同步与自动更新」接入本地 Hook 服务与自定义域名。见 `SKILL.md` 的「基线 vs 完整版」。
